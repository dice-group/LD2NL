...
public class TripleConverter {
  ...
  public TripleConverter(SparqlEndpoint endpoint, String cacheDirectory,
    ...
    this(
        new QueryExecutionFactoryHttp(endpoint.getURL().toString(), endpoint.getDefaultGraphURIs()),
        null, null, cacheDirectory, wordnetDirectory, lexicon);
  }

  public TripleConverter(QueryExecutionFactory qef, IRIConverter uriConverter,
      String cacheDirectory, Dictionary wordnetDirectory) {
  ...
  public TripleConverter(QueryExecutionFactory qef, PropertyVerbalizer propertyVerbalizer,
      IRIConverter uriConverter, String cacheDirectory, Dictionary wordnetDirectory,
      Lexicon lexicon) {
  ...
  /**
   * Return a textual representation for the given triples. Currently we assume that all triples
  ...
  public SPhraseSpec convertToPhrase(Triple t, boolean negated, boolean reverse) {
    ...
    if (predicate.isVariable()) {
      ...
      if (objectIsClass) {
        ...
        if (type == PropertyVerbalizationType.NOUN) {
          ...
          // simple heuristic: OBJECT is variable and predicate is of type owl:FunctionalProperty or
          // rdfs:range is xsd:boolean
          ...
          List<org.aksw.triple2nl.nlp.relation.Pattern> l =
              BoaPatternSelector.getNaturalLanguageRepresentation(predicate.toString(), 1);
          if (l.size() > 0) {
    ...
    // check if the meaning of the triple is it's negation, which holds for boolean properties with
    // FALSE as value
    if (!negated) {
  ...
  private boolean usePluralForm(Triple triple) {
    return triple.getObject().isVariable() && !(reasoner
        .isFunctional(new OWLObjectPropertyImpl(IRI.create(triple.getPredicate().getURI())))
  ...
  /**
   * @param returnAsSentence whether the style of the returned result is a proper English sentence
   *        or just a phrase
   */
  ...
  /**
   * Process the node and return an NLG element that contains the textual representation. The output
   * depends on the node type, i.e. variable, URI or literal.
   * 
  ...
  /**
   * Converts the node that is supposed to represent a class in the knowledge base into an NL
   * phrase.
   * 
   * @param node the node
  ...
  public NPPhraseSpec processVarNode(Node varNode) {
    return nlgFactory
        .createNounPhrase(nlgFactory.createWord(varNode.toString(), LexicalCategory.NOUN));
  }
  ...
  public NPPhraseSpec processLiteralNode(Node node) {
    ...
    NPPhraseSpec np =
        nlgFactory.createNounPhrase(nlgFactory.createWord(literalText, LexicalCategory.NOUN));
