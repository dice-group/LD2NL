...
public class OWLClassExpressionConverterTest_Extended {
  ...
  public void testNested1() {
    ...
    ce = df.getOWLObjectMinCardinality(5, worksFor, df.getOWLObjectIntersectionOf(company,
        df.getOWLObjectSomeValuesFrom(ledBy, df.getOWLObjectUnionOf(company, person))));
    text = converter.convert(ce);
    System.out.println(ce + " = " + text);
    // String expected = "someone who works for at least 5 companies that is ledby a company or a
    // person";
    // Assert.assertEquals(expected, text);

    /* someone who works for at least one company which is led by a company or a person */
    ce = df.getOWLObjectMinCardinality(1, worksFor, df.getOWLObjectIntersectionOf(company,
        df.getOWLObjectSomeValuesFrom(ledBy, df.getOWLObjectUnionOf(company, person))));
    text = converter.convert(ce);
    System.out.println(ce + " = " + text);
  }
  ...
  public void simpleNegation() {
    ...
    ce = df.getOWLObjectComplementOf(
        df.getOWLObjectSomeValuesFrom(worksFor, df.getOWLObjectUnionOf(person, company)));
    text = converter.convert(ce);
    System.out.println(ce + " = " + text);
    // final String expected = "someone who does not work for a person or a company";
    // Assert.assertEquals(expected, text);
  }
  ...
  public void complexNegationWithMaxCardinality() {
    ...
    System.out.println(ce + " = " + text);
    System.out.println(
        "Expected : someone who does not work for a person or a company and whose birthplace is paderborn that has not more than 50000 inhabitants");
  }
  ...
  public void testNested1WithNegation() {
    ...
    System.out.println(ce + " = " + text);
    System.out.println(
        "Expected: someone who works for at least 5 companies which are not software companies and are ledby a company or a person");
    ...
    System.out.println(ce + " = " + text);
    System.out.println(
        "Expected: someone who works for at least one company which is not a software company and led by a company or a person");
  }
  ...
  public void NestedtesthasValue() {
    ...
    final String expected = "something that works place something that works place Paderborn";
    Assert.assertEquals(expected, text);
    // LOG.info(ce + " = " + text);
  }
  ...
  public void testComplex() {
    ...
    complexce1 = df.getOWLObjectUnionOf(
    ...
    ce = df.getOWLObjectIntersectionOf(df.getOWLObjectIntersectionOf(
    ...
    complexce2 = df.getOWLObjectIntersectionOf(
        df.getOWLObjectIntersectionOf(df.getOWLObjectHasValue(sings, karaoke), person),
        df.getOWLObjectIntersectionOf(df.getOWLObjectHasValue(sings, Jazz), person));
    text = converter.convert(ce);
    System.out.println(ce + "=" + text);
    ...
    ce = df.getOWLObjectIntersectionOf(
        df.getOWLObjectIntersectionOf(df.getOWLObjectHasValue(sings, rock), person), complexce1);
    text = converter.convert(ce);
    System.out.println(ce + "=" + text);

    ce = df.getOWLObjectUnionOf(
        df.getOWLObjectIntersectionOf(df.getOWLObjectHasValue(plays, Cricket), person), complexce2);
    text = converter.convert(ce);
    System.out.println(ce + "=" + text);
